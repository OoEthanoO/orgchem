...
`/practice` shows one of 283 structures and asks for its IUPAC name, filtered by topic
(alkanes, alkenes, haloalkanes, alcohols, carbonyls, acids, amines, aromatics)
and by level. Three hints are available one at a time: what class of compound it
is, how big the parent is and what hangs off it, and finally the name with only
its numbers removed — which leaves exactly the part the exercise is about.

Answers are marked by resolving what you type back into a structure and
comparing it with the one shown, rather than by matching a string. Any name that
