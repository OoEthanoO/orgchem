...
name is the structure it was paired with. Organic difficulty is computed from
the naming decisions required. Hard combines branching, functional-group
priorities, detailed numbering and explicitly specified stereochemistry.
Molecule size alone and unspecified stereocentres do not make a question Hard.
Routine questions remain available at Easy or Medium. Hard multiple-choice
questions prefer same-family, same-formula isomers, with other isomers or
similar-sized compounds from the same topic filling the remaining options.
...
Advanced candidates live in `scripts/quiz-hard-candidates.mjs`. Use `--hard-only`
instead of `--expand` to verify and append those; only pairs whose verified name
and structure meet the Hard criteria are kept.
...
with positive, zero and negative metal oxidation states. Hard combines ligand
ordering, chelate prefixes and charge balancing; simple single-ligand ions or
ordinary salts belong at Easy or Medium. Independent formula and
